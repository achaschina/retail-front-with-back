...
module.exports = (app) => {
  const api = app.RetailAPI.app.api.items;

  app.route('/api/v1/items')
    //  .post(passport.authenticate('jwt', config.session), api.store(models.User, models.Budget, models.Client, app.get('budgetsecret')))
     .get(passport.authenticate('jwt', config.session), api.getAll(models.User, models.Items, app.get('budgetsecret')))
  ...
  app.route('/api/v1/items/single')
     .get(passport.authenticate('jwt', config.session), api.index(models.User, models.Items, app.get('budgetsecret')))
